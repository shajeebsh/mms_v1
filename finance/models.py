...
class Expense(models.Model):
    ...
    name = models.CharField(
        max_length=200,
        blank=True,
        help_text="Name of the person/entity related to this expense",
    )
    category = models.ForeignKey(
    ...
    def __str__(self):
        if self.name:
            return f"Expense: {self.name} - {self.description[:30]} - ₹{self.amount}"
        return f"Expense: {self.description[:50]} - ₹{self.amount}"
